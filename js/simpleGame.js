...
var Timer = function()
{
   ...
   this.getTotalTime = function(){
    this.currentTime = Date.now();
    return (this.currentTime - this.date);
   }
}
...
function Sprite(scene, imageFile, width, height, type){ 
  ...
  this.checkBounds = function(){
  	...
  	leftBorder = camX;
  	topBorder = camY;
    //bottomborder - 30 because of healthbar
  	bottomBorder = this.cHeight + camY - 30;
    /* ---------- PLAYER BOUNDS ---------- */
    if(this.type == 'player') {
    ...
    /* ---------- LASER BOUNDS ---------- */
    if(this.type == 'laser') {
        //if laser leaves any boundary, set its outofbounds to true (for it to be removed)
        if (this.x > rightBorder || this.y > bottomBorder
              || this.x < leftBorder || this.y < topBorder){
          this.outOfBounds = true;
        }
    }
    /* ---------- END LASER BOUNDS ---------- */
    /* ---------- ASTEROID BOUNDS ---------- */
    //if asteroid leaves boundary, reverse its direction
    if(this.type == 'asteroid' || this.type == 'small asteroid') {
      //originally just multiplied dx/dy by -1 but stuff would get stuck on border
  ...
  this.report = function(){
    ...
    */
function Scene(pixelWidth, pixelHeight, width, height, left, top){
...
K_LEFT = 37; K_RIGHT = 39; K_UP = 38;K_DOWN = 40; K_SPACE = 32; K_ENTER = 13;
//Animation Constants
